...
func _process(delta):
	if global_position.distance_to(get_tree().get_first_node_in_group("Player").global_position + Go_To) > 0.5:
...
func _physics_process(delta):
	...
	if global_position.distance_to(get_tree().get_first_node_in_group("Player").global_position) > 1:
		...
		if player:
			var direction = (player.global_position + Go_To - global_position ).normalized()
			var speed = 3.25 # Imposta la velocità desiderata
